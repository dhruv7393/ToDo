...
import { Grid, Row, Button, notification, List, Typography, Drawer, Checkbox, Switch, Badge } from 'antd';
...
const ShowTasks = () => {
    ...
    const handleVaccationSelected = (value) =>{
      ...
      let config = {
        method: 'patch',
        url: process.env.REACT_APP_BACKEND_URL + 'vaccation/'+onVaccationDetails["_id"],
        headers: { 
      ...
      axios(config)
        .then( (response) =>{
          openNotification("Hello Dhruv!",`Vaccation has been ${value ? 'enabled': 'disabled'}`)
    ...
    const handleDelete = (index) =>{
        ...
        let config = {
          method: 'delete',
          url: process.env.REACT_APP_BACKEND_URL+'copythat/'+arrayWithoutDeletedTodo[index]["_id"],
          headers: { }
        };
        
        axios(config)
        .then(function (response) {
          openNotification(arrayWithoutDeletedTodo[index]["taskName"],`It has been deleted`)
          arrayWithoutDeletedTodo.splice(index, 1)
          setTodos(sortData(arrayWithoutDeletedTodo))
        })
        .catch(function (error) {
          console.log("Error:",error);
          openNotification(arrayWithoutDeletedTodo[index]["taskName"],`Could not be deleted`, true)
        });
    }

    const updateTodo = (index, updatedValue, method, successMessage, errorMessage) =>{
      let arrayWithoutDeletedTodo = [...todos]
      let removedTodo = arrayWithoutDeletedTodo.splice(index, 1)
      console.log(updatedValue["_id"])

      let data = JSON.stringify(updatedValue);
        
      var config = {
        method: method,
        url: process.env.REACT_APP_BACKEND_URL+'copythat/'+updatedValue["_id"],
        headers: { 
          'content-type': 'application/json'
        },
        data : data
    ...
    const updatedTaskDetail = (index, value) =>{
      ...
      updateTodo(index, todoToBeModified, "patch", `Detail set to - ${value}`, `Could not update details`)
    }
    ...
    const changeCount = (index, value) =>{
      ...
      updateTodo(index, todoToBeModified, "patch", `Count has been updated to ${todoToBeModified.count}`, `Could not update count`)
    }

    const updatedRepeatDays = (index, value) =>{
      let todoToBeModified = todos[index]
      todoToBeModified.repeatOn =  value.length ? value : todoToBeModified.repeatOn

      if(!value.length){
        openNotification(todoToBeModified.taskName,`At least should be repeated for a day`, true)
      }else{
        updateTodo(index, todoToBeModified, "patch", `Will be counted on ${todoToBeModified.repeatOn}`, `Could not update counting days`)
      }
    ...
    const updatedRepeat= (index, value) =>{
      ...
      todoToBeModified.canBeRepeated = value>0
      updateTodo(index, todoToBeModified, "patch", value==0? 'Get it done!!!' : `Task will be repeated after ${value} days`, `Could not marked for once / repetition`)
    }

    const changeImportance = (index, value) =>{
      let todoToBeModified = todos[index]
      todoToBeModified.importance = value > 5 ? 1 : value<1 ? 1 : value
      updateTodo(index, todoToBeModified, "patch", `Importance set to ${todoToBeModified.importance}`, `Could not update importance`)
    }

    const handleDone = (index) =>{
      let todoToBeModified = todos[index]
      todoToBeModified.done = !todoToBeModified.done
      updateTodo(index, todoToBeModified, "patch", `It has been marked as done`, `Could not be marked as done`)
    }

    const handlePinned = (index) =>{
        let todoToBeModified = todos[index]
        todoToBeModified.pinned = !todoToBeModified.pinned
        updateTodo(index, todoToBeModified, "patch", `It has been pinned`, `Could not be pinned`)
    }
    ...
    useEffect(()=>{
        ...
        .catch((err) => {
          ...
          openNotification("Vaccation details",`Failed to load vaccation details`, true)
        });
        ...
        .catch((err) => {
          console.log("Error:", err)
          openNotification("Cannot load ToDos",`Please check error logs`, true)
        });
    ...
    return (
        ...
        visible={visible}
      >
        Vaccation since {onVaccationDetails.startedFrom}: <Switch defaultChecked={onVaccationDetails.onVaccation} value={onVaccationDetails.onVaccation} onChange={(checked)=>handleVaccationSelected(checked)} /> <Badge count={onVaccationDetails.vaccationDays} />
        <List
        ...
        {contextHolder}
          <ModalToAddTodo openModalToAdd={openModalToAdd} setOpenModalToAdd={setOpenModalToAdd} addTodo={addTodo} />
            <Row gutter={16}>
                {todos.length && todos.map((todo, index)=><DisplayIndividualTask todoToBeDisplayed={todo} cardWidth={cardWidth} checkedItems={checkedItems} index={index} changeCount={changeCount} handleDone={handleDone} handlePinned={handlePinned} changeImportance={changeImportance} updatedRepeatDays={updatedRepeatDays} updatedRepeat={updatedRepeat} updatedTaskDetail={updatedTaskDetail} handleDelete={handleDelete} />) || <></>}
        </Row>
        </div>
    )
}
