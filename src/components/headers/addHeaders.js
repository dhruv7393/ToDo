...
function AddHeader(props) {
    ...
    const addHeader = () =>{
        if(headerIP in headers){
            ...
            axios.post(process.env.REACT_APP_BACKEND_URL + "headers", {title: headerIP, pinned})
            .then(data => {
  ...
  return (
    <Dialog onClose={handleClose} open={open} sx={{ m: 0, p: 2 }} id="customized-dialog-title">
      ...
      <DialogContent>
        <TextField id="outlined-basic" variant="outlined" defaultValue={headerIP} onChange={e => addHeaderIP(e.target.value)} fullWidth autoFocus />
        Pinned: <Switch aria-label="pinned" checked={pinned} onClick={() =>setPinned(!pinned)} />
      </DialogContent>
      <DialogActions>
        <Button variant="contained" onClick={() => addHeader()}>Submit</Button>
      </DialogActions>
    </Dialog>
  );
}
