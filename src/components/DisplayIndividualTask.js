...
const DisplayIndividualTask = ({
  ...
  let toBeDisplayed =
    (checkedItems["Countable"] ? canBeCounted : true) &&
    (checkedItems["Repetable"] ? canBeRepeated : true) &&
    (checkedItems["Pinned"] ? pinned : true) &&
    (checkedItems["Done"] ? done : true) &&
    (checkedItems["Later"] ? setForLater : true);
        ...
        <Col span={cardWidth} key={_id}>
          <Card
              ...
              (isCountable && (
                <>
                  <MinusCircleOutlined
                    ...
                    style={{ color: count < 0 ? "#ff4d4f" : "#1677ff" }}
                  />
                  <Button
                    type="text"
                    style={{ color: count < 0 ? "#ff4d4f" : "#1677ff" }}
            ...
            actions={[
              ...
              <Button
                key="pinned"
                onClick={() => handlePinned(index)}
                disabled={isCountable}
                type={pinned || isCountable ? "primary" : "text"}
                shape="circle"
              >
                <PushpinOutlined />
              </Button>,
              <Button
                key="star"
                onClick={() => changeImportance(index, importance + 1)}
                disabled={isCountable}
                type="text"
                shape="circle"
              >
              ...
              (!editRepeat && !editRepeatDays && (
                <Button
                  key="Repeat"
                  onClick={() => {
                    if (isCountable) {
                      setEditRepeatDays(true);
                ...
                (editRepeatDays && (
                  ...
                  <>
                    <InputNumber
                      defaultValue={repeatUpdated}
                      onChange={(value) => setRepeatUpdated(value)}
                      min={0}
                      max={366}
                    />
                    <Button
              ...
              <Button
                ...
                disabled={isCountable || !done || canBeRepeated}
                type="text"
                shape="circle"
              >
                <DeleteOutlined />
          ...
          >
            {editTaskDeatil ? (
              <Input
                value={taskDetailUpdated}
                onChange={(e) => setTaskDeatilUpdated(e.target.value)}
              />
            ) : (
              taskDetail
            )}
