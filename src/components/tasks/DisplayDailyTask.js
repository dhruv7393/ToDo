              ...
              return (
                ...
                >
                  <CardHeader
                    ...
                    title={title==="Goon"? <LightTooltip title={textToBeDisplayed(pending)}> <Button>{title}</Button></LightTooltip> : title}
                  />
                </Card>
              );
          })) || <></>}
...
const textToBeDisplayed = (pending) =>{
  ...
  let valueToBeCalculated = ((Math.ceil(
    (new Date(new Date().toLocaleDateString()) - new Date('4/30/2024')) /
      (1000 * 60 * 60 * 24)
  )) +pending)%22
